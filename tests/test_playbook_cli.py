...
def test_phase6_parser_accepts_exact_commands() -> None:
    ...
    build = parser.parse_args(["playbook-build", "--file", "x.jsonl", "--knowledge-index", "k", "--output", "o"])
    smoke = parser.parse_args(["playbook-smoke", "--knowledge-index", "k", "--playbooks", "p", "--cases", "c", "--work-directory", "w", "--report", "r"])
...
def test_validate_output_safe_and_no_ollama(monkeypatch, capsys) -> None:
    monkeypatch.setattr(cli, "load_playbooks", lambda path: [
        {"status":"APPROVED"},{"status":"APPROVED"},{"status":"DRAFT"},{"status":"RETIRED"}
    ])
    monkeypatch.setattr(cli, "OllamaClient", lambda *a, **k: (_ for _ in ()).throw(AssertionError("Ollama forbidden")))
...
def test_build_and_smoke_output_safe_and_no_ollama(monkeypatch, capsys, tmp_path: Path) -> None:
    monkeypatch.setattr(cli, "build_playbook_catalog", lambda *a, **k: {
        "domain":"APPROVED_PLAYBOOK","approved_playbooks":2,"active_links":3
    })
    monkeypatch.setattr(cli, "run_playbook_smoke", lambda *a, **k: {"ok":True,"cases":[{}]*10})
    monkeypatch.setattr(cli, "OllamaClient", lambda *a, **k: (_ for _ in ()).throw(AssertionError("Ollama forbidden")))
    assert cli.main(["playbook-build","--file","p","--knowledge-index","k","--output","o"]) == 0
    assert cli.main(["playbook-smoke","--knowledge-index","k","--playbooks","p","--cases","c","--work-directory","w","--report",str(tmp_path/'r.json')]) == 0
    out = capsys.readouterr().out
