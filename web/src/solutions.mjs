...
function renderTopicCard([key, label], selectedCategory, count = 0) {
  const [icon, description] = TOPIC_PRESENTATION[key];
  const countLabel = count === 1 ? '1 orientação' : `${count} orientações`;
  return `<button class="faq-topic-card" type="button" data-category="${key}" aria-pressed="${selectedCategory === key}" aria-controls="faq-results">${topicIcon(icon)}<span class="faq-topic-copy"><strong>${escapeHtml(label)}</strong><span>${escapeHtml(description)}</span><small class="faq-topic-count">${escapeHtml(countLabel)}</small></span><span class="faq-topic-arrow" aria-hidden="true">→</span></button>`;
}

function trustSignal(icon, title, description) {
  return `<div class="help-trust-item"><span class="help-trust-icon" aria-hidden="true">${icon}</span><span><strong>${escapeHtml(title)}</strong><small>${escapeHtml(description)}</small></span></div>`;
}
...
export function renderSolutionsHome(options = {}) {
  ...
  const topicCounts = new Map(
    (options.groups ?? []).map(group => [group.key, group.items?.length ?? 0]),
  );
  return `<section class="solutions-home" aria-labelledby="solutions-title">
    <div class="help-intro">
      ...
      <div class="help-trust-strip" aria-label="Como o Jup Resolve protege a orientação">${trustSignal('✓', 'Base aprovada', 'Orientações revisadas antes da demonstração.')}${trustSignal('◇', 'Identidade da sessão', 'A conversa não muda quem você é no sistema.')}${trustSignal('→', 'Próximo passo claro', 'Quando necessário, o Jup encaminha o atendimento.')}</div>
    </div>
    <section class="faq-discovery" aria-labelledby="topic-title"><div class="faq-discovery__heading"><div><span class="section-label">Atalhos por assunto</span><h2 id="topic-title">Comece pelo que está impedindo seu trabalho</h2></div><button class="faq-topic-all" type="button" data-category="" aria-pressed="${selectedCategory === ''}" aria-controls="faq-results">Todos os tópicos <span aria-hidden="true">↗</span></button></div><div class="faq-topic-grid">${FAQ_TOPICS.map(topic => renderTopicCard(topic, selectedCategory, topicCounts.get(topic[0]) ?? 0)).join('')}</div></section>
    ...
    <aside class="jup-spotlight"><div class="jup-spotlight__mark" aria-hidden="true">${renderJupVisual({ state: 'idle' })}</div><div class="jup-spotlight__copy"><div class="jup-spotlight__status"><span aria-hidden="true"></span>Disponível para conversar</div><h2>Não encontrou o que precisa?</h2><p>Conte o que aconteceu. O Jup começa pelo seu problema e conduz o próximo passo.</p></div><a href="/jup" data-route="jup">Falar com o Jup <span aria-hidden="true">→</span></a></aside>
  </section>`;
}
...
export function renderSolutionDetail(detail) {
  ...
  const officialAction = detail.procedure_url
    ? `<section class="solution-official-card"><span class="solution-official-card__icon" aria-hidden="true">↗</span><div><span>Destino oficial</span><strong>${escapeHtml(detail.system || 'Sistema corporativo')}</strong><p>Abra somente o endereço validado pelo conteúdo aprovado.</p></div><a class="solution-official-action" href="${escapeHtml(detail.procedure_url)}" target="_blank" rel="noopener noreferrer">Abrir canal oficial <span aria-hidden="true">↗</span></a></section>`
    : '';
  return `<article class="solution-detail">
    ...
    <div class="solution-detail-layout">
      ...
      <aside class="solution-detail-aside"><section class="solution-meta-card"><div class="solution-meta-status"><span aria-hidden="true"></span><strong>Conteúdo aprovado</strong></div><dl><div><dt>Sistema</dt><dd>${escapeHtml(detail.system || 'Corporativo')}</dd></div><div><dt>Categoria</dt><dd>${escapeHtml(category)}</dd></div>${version ? `<div><dt>Versão</dt><dd>${escapeHtml(version)}</dd></div>` : ''}<div><dt>Canal</dt><dd>${detail.procedure_url ? 'Link oficial validado' : 'Orientação interna'}</dd></div></dl>${detail.provenance ? `<p class="knowledge-provenance">Demonstração · ${escapeHtml(detail.provenance.source)} · ${escapeHtml(detail.provenance.status)}<br><small>${escapeHtml(detail.knowledge_id)}</small></p>` : ''}</section>${officialAction}<section class="solution-context-card"><span class="solution-context-icon" aria-hidden="true">↳</span><div><strong>Precisa continuar?</strong><p>O Jup pode usar esta orientação como contexto da conversa.</p></div></section><section class="solution-trust-card"><span aria-hidden="true">✓</span><p>Esta orientação informa o caminho. Ela não concede acesso nem altera permissões por conta própria.</p></section></aside>
    </div>
  </article>`;
}
